...
k0 = 1; 
pval = 0.05; 
nIters = 200; % Number of Monte Carlo replications (use 1000 for final results)
...
for nt = 1:length(n_values)
    n = n_values(nt);
    T = T_values(nt);
    disp(['Starting simulations for n = ', num2str(n), ', T = ', num2str(T)]);
    ...
    while r <= 5 % Number of factors
        ...
        while DGP <= 3 % DGP from 1 to 3 
            ...
            while thetaMethod <= 4 % Theta, the signal-to-noise parameter
                disp(['Processing r = ', num2str(r), ', DGP = ', num2str(DGP), ...
                ...
                parfor k = 1:nIters % Monte Carlo replications
                    try
                        ...
                        disp(['Error in replication ', num2str(k), ...
                              ', r = ', num2str(r), ', DGP = ', num2str(DGP), ...
                              ', thetaMethod = ', num2str(thetaMethod), ...
                              ', n = ', num2str(n), ', T = ', num2str(T), ...
                              ': ', e.message]);
                        out(k, :) = [NaN, NaN];
                ...
                success_rate = mean(out == r, 'omitnan');
                disp(['Completed: r = ', num2str(r), ', DGP = ', num2str(DGP), ...
                      ', thetaMethod = ', num2str(thetaMethod), ...
                      ', n = ', num2str(n), ', T = ', num2str(T), ...
                      ', Success Rates (Seq, Eigen) = [', ...
                      num2str(success_rate(1)), ', ', num2str(success_rate(2)), ']']);
                ...
                if n == 50 && T == 50
                    ...
                    if thetaMethod == 1
                        finalTable.sequentialTest_n50_T50.rx05(index) = mean(out(:,1)==r, 'omitnan');
                        finalTable.eigenvalueRatio_n50_T50.rx05(index) = mean(out(:,2)==r, 'omitnan');
                    elseif thetaMethod == 2
                        finalTable.sequentialTest_n50_T50.rx1(index) = mean(out(:,1)==r, 'omitnan');
                        finalTable.eigenvalueRatio_n50_T50.rx1(index) = mean(out(:,2)==r, 'omitnan');
                    elseif thetaMethod == 3
                        finalTable.sequentialTest_n50_T50.rx3(index) = mean(out(:,1)==r, 'omitnan');
                        finalTable.eigenvalueRatio_n50_T50.rx3(index) = mean(out(:,2)==r, 'omitnan');
                    elseif thetaMethod == 4
                        finalTable.sequentialTest_n50_T50.rx5(index) = mean(out(:,1)==r, 'omitnan');
                        finalTable.eigenvalueRatio_n50_T50.rx5(index) = mean(out(:,2)==r, 'omitnan');
                    end
                elseif n == 50 && T == 100
                    ...
                    if thetaMethod == 1
                        finalTable.sequentialTest_n50_T100.rx05(index) = mean(out(:,1)==r, 'omitnan');
                        finalTable.eigenvalueRatio_n50_T100.rx05(index) = mean(out(:,2)==r, 'omitnan');
                    elseif thetaMethod == 2
                        finalTable.sequentialTest_n50_T100.rx1(index) = mean(out(:,1)==r, 'omitnan');
                        finalTable.eigenvalueRatio_n50_T100.rx1(index) = mean(out(:,2)==r, 'omitnan');
                    elseif thetaMethod == 3
                        finalTable.sequentialTest_n50_T100.rx3(index) = mean(out(:,1)==r, 'omitnan');
                        finalTable.eigenvalueRatio_n50_T100.rx3(index) = mean(out(:,2)==r, 'omitnan');
                    elseif thetaMethod == 4
                        finalTable.sequentialTest_n50_T100.rx5(index) = mean(out(:,1)==r, 'omitnan');
                        finalTable.eigenvalueRatio_n50_T100.rx5(index) = mean(out(:,2)==r, 'omitnan');
                    end
                else
                    warning(['No storage defined for n = ', num2str(n), ', T = ', num2str(T)]);
                end
    ...
    disp(['Finished simulations for n = ', num2str(n), ', T = ', num2str(T)]);
end

% Verify stored results
disp('Verifying stored results:');
disp('n=50, T=50 (Sequential Test):');
disp(finalTable.sequentialTest_n50_T50);
disp('n=50, T=50 (Eigenvalue Ratio):');
disp(finalTable.eigenvalueRatio_n50_T50);
disp('n=50, T=100 (Sequential Test):');
disp(finalTable.sequentialTest_n50_T100);
disp('n=50, T=100 (Eigenvalue Ratio):');
disp(finalTable.eigenvalueRatio_n50_T100);
